...
mx_status_t GpuDevice::virtio_gpu_set_mode(mx_device_t* dev, mx_display_info_t* info) {
    ...
    GpuDevice* gd = static_cast<GpuDevice*>(d);
...
GpuDevice::GpuDevice(mx_driver_t* driver, mx_device_t* bus_device)
    ...
    mtx_init(&request_lock_, mtx_plain);
    cnd_init(&request_cond_);
    mtx_init(&flush_lock_, mtx_plain);
...
GpuDevice::~GpuDevice() {
    ...
    mtx_destroy(&request_lock_);
    cnd_destroy(&flush_cond_);
    mtx_destroy(&flush_lock_);
}
...
mx_status_t GpuDevice::send_command_response(const void* cmd, size_t cmd_len, void** _res, size_t res_len) {
    ...
    *_res = res;
    ...
    /* wait for result */
    cnd_wait(&request_cond_, &request_lock_);
...
mx_status_t GpuDevice::attach_backing(uint32_t resource_id, mx_paddr_t ptr, size_t buf_len) {
    LTRACEF("dev %p, resource_id %u, ptr %#lx, buf_len %zu\n", this, resource_id, ptr, buf_len);
...
void GpuDevice::Flush() {
    mtx_lock(&flush_lock_);
    flush_pending_ = true;
    cnd_signal(&flush_cond_);
    mtx_unlock(&flush_lock_);
}
...
void GpuDevice::virtio_gpu_flusher() {
    ...
    for (;;) {
        mtx_lock(&flush_lock_);
        if (!flush_pending_)
            cnd_wait(&flush_cond_, &flush_lock_);
...
mx_status_t GpuDevice::virtio_gpu_start() {
    ...
    err = mx_alloc_device_memory(get_root_resource(), (uint32_t)len, &fb_pa_, (void**)&fb_);
    if (!fb_) {
...
mx_status_t GpuDevice::Init() {
    ...
    auto r = mx_alloc_device_memory(get_root_resource(), PAGE_SIZE, &gpu_req_pa_, (void**)&gpu_req_);
    if (r < 0) {
        VIRTIO_ERROR("cannot alloc gpu_req buffers %d\n", r);
        return r;
    }

    LTRACEF("allocated gpu request at %p, physical address 0x%lx\n", gpu_req_, gpu_req_pa_);
...
void GpuDevice::IrqRingUpdate() {
    ...
    auto free_chain = [this](vring_used_elem* used_elem) {
        ...
        mtx_lock(&request_lock_);
        cnd_signal(&request_cond_);
        mtx_unlock(&request_lock_);
    };
