...
VmAddressRegion::VmAddressRegion(VmAspace& aspace, vaddr_t base, size_t size, uint32_t vmar_flags)
    : VmAddressRegionOrMapping(kMagic, base, size, vmar_flags, aspace, nullptr, "root") {
...
status_t VmAddressRegion::CreateSubVmarInternal(size_t offset, size_t size, uint8_t align_pow2,
    ...
    bool is_specific_overwrite  = !!(vmar_flags & VMAR_FLAG_SPECIFIC_OVERWRITE);
    bool is_specific = !!(vmar_flags & VMAR_FLAG_SPECIFIC) || is_specific_overwrite;
    ...
    if (is_specific) {
        ...
        // This is a hack for the migration to the new interface.  It works
        // around the lack of VALLOC_BASE until things start using subregions.
        if (vmar_flags & VMAR_FLAG_MAP_HIGH) {
            static const vaddr_t mmio_map_base_address = _LP64 ? 0x7ff000000000ULL : 0x20000000UL;
